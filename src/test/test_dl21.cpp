...
namespace groupsig {
  ...
  // The fixture for testing DL21 scheme.
  class DL21Test : public ::testing::Test {
  protected:
    ...
    DL21Test() {
      ...
      rc = groupsig_init(GROUPSIG_DL21_CODE, time(NULL));
      EXPECT_EQ(rc, IOK);
  
      isskey = groupsig_mgr_key_init(GROUPSIG_DL21_CODE);
      EXPECT_NE(isskey, nullptr);

      grpkey = groupsig_grp_key_init(GROUPSIG_DL21_CODE);
	...
	free(memkey); memkey = NULL;
      ...
      groupsig_clear(GROUPSIG_DL21_CODE);      
    }
  ...
  TEST_F(DL21Test, GetCodeFromStr) {

    int rc;
    uint8_t scheme;

    rc = groupsig_get_code_from_str(&scheme, (char *) GROUPSIG_DL21_NAME);
    EXPECT_EQ(rc, IOK);

    EXPECT_EQ(scheme, GROUPSIG_DL21_CODE);

  }

  // Tests that the DL21 constructor creates the required keys.
  TEST_F(DL21Test, CreatesGrpAndMgrKeys) {

    /* Scheme is set to DL21 */
    EXPECT_EQ(grpkey->scheme, GROUPSIG_DL21_CODE);
    EXPECT_EQ(isskey->scheme, GROUPSIG_DL21_CODE);
    
  }

  /* groupsig_get_joinstart must return 0 */
  TEST_F(DL21Test, CheckJoinStart) {

    int rc;
    uint8_t start;
    
    rc = groupsig_get_joinstart(GROUPSIG_DL21_CODE, &start);
    EXPECT_EQ(rc, IOK);
  ...
  TEST_F(DL21Test, CheckJoinSeq) {

    int rc;
    uint8_t seq;
    
    rc = groupsig_get_joinseq(GROUPSIG_DL21_CODE, &seq);
    EXPECT_EQ(rc, IOK);
  ...
  TEST_F(DL21Test, AddsNewMember) {

    int rc;

    rc = groupsig_setup(GROUPSIG_DL21_CODE, grpkey, isskey, NULL);
    EXPECT_EQ(rc, IOK);

    addMembers(1);

    EXPECT_EQ(memkey[0]->scheme, GROUPSIG_DL21_CODE);    

  }

  /* Successfully initializes a signature */
  TEST_F(DL21Test, InitializeSignature) {
    ...
    EXPECT_EQ(sig->scheme, GROUPSIG_DL21_CODE);
  ...
  TEST_F(DL21Test, SignVerifyValid) {
    ...
    /* 
       ...
       (DL21 messages are JSON objects with scope and message) 
    */
  ...
  TEST_F(DL21Test, SignVerifyWrongMessage) {
  ...
  TEST_F(DL21Test, SuccessfullyLinkSigsSameUser) {
  ...
  TEST_F(DL21Test, FailsLinkSigsDifferentUsers) {
  ...
  TEST_F(DL21Test, GrpKeyExportImport) {
    ...
    dst = groupsig_grp_key_import(GROUPSIG_DL21_CODE, bytes, size);
  ...
  TEST_F(DL21Test, GrpKeyCopy) {
    ...
    dst = groupsig_grp_key_init(GROUPSIG_DL21_CODE);
    EXPECT_NE(dst, nullptr);
  ...
  TEST_F(DL21Test, IssKeyExportImport) {
    ...
    /* Import the group key */
    dst = groupsig_mgr_key_import(GROUPSIG_DL21_CODE, bytes, size);
    EXPECT_NE(dst, nullptr);
  ...
  TEST_F(DL21Test, IssKeyCopy) {
    ...
    dst = groupsig_mgr_key_init(GROUPSIG_DL21_CODE);
    EXPECT_NE(dst, nullptr);
  ...
  TEST_F(DL21Test, MemKeyExportImport) {
    ...
    /* Import the group key */
    dst = groupsig_mem_key_import(GROUPSIG_DL21_CODE, bytes, size);
    EXPECT_NE(dst, nullptr);
  ...
  TEST_F(DL21Test, MemKeyCopy) {
   ...
   /* Add one member */
    ...
    dst = groupsig_mem_key_init(GROUPSIG_DL21_CODE);
    EXPECT_NE(dst, nullptr);
  ...
  TEST_F(DL21Test, SignatureToString) {
  ...
  TEST_F(DL21Test, SignatureCopy) {
    ...
    /* Successfully creates a valid signature */
  TEST_F(DL21Test, SignatureExportImport) {
