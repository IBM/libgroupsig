...
char* message_misc_uint322string(uint32_t d) {
...
char* message_misc_uint642string(uint64_t d) {
  ...
  su = NULL; su_len = MAX_SUINT64;
  rc = INT_MAX;
  while(rc >= su_len) {

    if(!(su = (char *) realloc((char *)su, sizeof(char)*su_len))) {
      LOG_ERRORCODE(&logger, __FILE__, "misc_uint642string", __LINE__, 
		    errno, LOGERROR);
      return NULL;
    }
    memset(su, 0, sizeof(char)*su_len);
    
    if((rc = snprintf(su, su_len, "%llu", d)) < 0) {
      LOG_ERRORCODE(&logger, __FILE__, "misc_uint642string", __LINE__, 
		    errno, LOGERROR);
      free(su); su = NULL;
      return NULL;
