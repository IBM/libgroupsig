...
int dl21_verify_link(uint8_t *ok,
  ...
  if(!ok || !proof || proof->scheme != GROUPSIG_DL21_CODE ||
     !grpkey || grpkey->scheme != GROUPSIG_DL21_CODE ||
     !msg || !sigs || !msgs || !n) {
    LOG_EINVAL(&logger, __FILE__, "dl21_verify_link", __LINE__, LOGERROR);
    return IERROR;
  }
  ...
  if(!(hscp = pbcext_element_G1_init())) GOTOENDRC(IERROR, dl21_verify_link);
  if(!(hscp_ = pbcext_element_G1_init())) GOTOENDRC(IERROR, dl21_verify_link);
  if(pbcext_element_G1_clear(hscp_) == IERROR) GOTOENDRC(IERROR, dl21_verify_link);
  if(!(nym_ = pbcext_element_G1_init())) GOTOENDRC(IERROR, dl21_verify_link);
  if(pbcext_element_G1_clear(nym_) == IERROR) GOTOENDRC(IERROR, dl21_verify_link);
  ...
  for (i=0; i<n; i++ ) {
...
#ifndef PROFILE
    /* Verify signature */
    if (dl21_verify(&_ok, sigs[i], msgs[i], grpkey) == IERROR)
      GOTOENDRC(IERROR, dl21_verify_link);

    if (!_ok)  GOTOENDRC(IOK, dl21_verify_link);
#endif
    ...
    /* "Accumulate" nym */
    dl21_sig = (dl21_signature_t *) sigs[i]->sig;
    if(pbcext_element_G1_add(nym_, nym_, dl21_sig->nym) == IERROR)
      GOTOENDRC(IERROR, dl21_verify_link);
